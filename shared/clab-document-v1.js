...
function applyProvider(provider,model){
  const p=$('provider'); if(provider&&p&&[...p.options].some(o=>o.value===provider)){
    p.value=provider;
    try{syncUIForProvider(provider);}catch(_){}
  }
...
function applyDocument(doc,fileName=''){
  ...
  lastScore=score; mainPlayerScore=score; lastProvider=doc.provider||null; lastModel=doc.model||null; lastConcept=String(doc.concept||'');
  try{lastMidiBytes=buildMidi(score);}catch(_){lastMidiBytes=null;}
  uploadedScore=doc.sourceScore?clone(doc.sourceScore):null;
  uploadedName=doc.sourceName||'';
  ...
  if($('conceptView'))$('conceptView').innerHTML=lastConcept?`<strong>Kompositionsidee:</strong><br>${lastConcept.replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c])).replace(/\n/g,'<br>')}`:'Keine Kompositionsidee gespeichert.';
  ...
  if($('uploadInfo'))$('uploadInfo').textContent=uploadedScore?`CLAB-Vorlage: ${uploadedName||uploadedScore.ti||'Vorlage'} (${noteCount(uploadedScore)} Noten)`:'Keine Vorlage in dieser CLAB-Datei.';
...
function installUi(){
  ...
  const tech=$('technicalSection')?.querySelector('.foldcontent');if(tech&&!$('clabCompatBuild')){const d=document.createElement('div');d.id='clabCompatBuild';d.className='uploadinfo';d.style.marginTop='12px';d.textContent='CLAB-Kompatibilität V1 · Native V5.0.11 Schema';tech.appendChild(d);}
  return true;
}
