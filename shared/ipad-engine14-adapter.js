...
function toObjectScore(score){
  const s=JSON.parse(JSON.stringify(score||{}));
  s.tr=Array.isArray(s.tr)?s.tr.map((t,i)=>({nm:t.nm||`Spur ${i+1}`,ch:Number(t.ch??i)%16,pg:Number(t.pg)||0,nt:(Array.isArray(t.nt)?t.nt:[]).map(n=>Array.isArray(n)?{t:Number(n[0])||0,d:Math.max(.03,Number(n[1])||.25),p:Number(n[2]),v:Math.max(1,Math.min(127,Number(n[3])||88)),st:Number(n[4])||0,g:Number(n[5])||.95}:n).filter(n=>Number.isFinite(Number(n.p))),ct:Array.isArray(t.ct)?t.ct:[]})):[];
  return typeof normalizeScore==='function'?normalizeScore(s):s;
}
function fixApiKeyFields(){['keyOpenAI','keyAnthropic','keyGemini'].forEach(id=>{const e=$(id);if(!e)return;e.type='text';e.autocomplete='off';e.setAttribute('autocapitalize','none');e.setAttribute('autocorrect','off');e.setAttribute('spellcheck','false');e.setAttribute('inputmode','text');e.setAttribute('data-1p-ignore','true');e.setAttribute('data-lpignore','true');e.setAttribute('data-form-type','other')})}
function simplifyCompositionUI(){
  const idea=$('compIdea');
  if(idea){const field=idea.closest('.field');const prev=field?.previousElementSibling;if(prev?.tagName==='H3'&&prev.textContent.trim()==='Kompositionsidee')prev.remove();field?.remove()}
  const task=$('compTask');
  const card=task?.closest('.card');
  const h2=card?.querySelector(':scope > h2');if(h2&&h2.textContent.trim()==='Kompositionsidee')h2.textContent='Kompositionsauftrag';
  if(task){const label=task.closest('.field')?.querySelector('label');if(label)label.remove()}
  document.querySelectorAll('.hint').forEach(h=>{if(h.textContent.includes('Vorlage, Einstellungen und Kompositionsidee gehören zusammen.'))h.textContent='Vorlage und Einstellungen gehören zusammen. Nach der Übernahme aus dem Experimentallabor bleibt die Vorlage im Player, bis eine neue Komposition erzeugt wurde.'});
}
function ensureConceptBox(){let box=$('engine14ConceptBox');if(box)return box;const task=$('compTask');const host=task?.closest('.field')?.parentElement;if(!host)return null;const h=document.createElement('h3');h.id='engine14ConceptHeading';h.textContent='Musikalischer Impuls';box=document.createElement('div');box.id='engine14ConceptBox';box.className='idea-box';box.style.whiteSpace='pre-wrap';box.textContent='Noch kein Impuls erzeugt.';task.closest('.field').after(h,box);return box}
...
function downloadDiagnostic(){const d=window.__compositionLabLastDiagnostic;if(!d){alert('Noch keine Diagnosedatei vorhanden. Bitte zuerst eine neue Komposition mit Engine Build 14 erzeugen.');return}const title=safeFilename(d?.result?.ti||state?.current?.ti||'Komposition'),stamp=new Date(d.createdAt||Date.now()).toISOString().replace(/[:.]/g,'-');const blob=new Blob([JSON.stringify(d,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`${title}-Diagnose-Engine14-${stamp}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),1500)}
function ensureDiagnosticButton(){let b=$('ipadDiagnosticDownload');if(!b){const save=$('saveMidi'),host=save?.parentElement;if(host){b=document.createElement('button');b.id='ipadDiagnosticDownload';b.className='btn';b.type='button';b.textContent='Diagnosedatei speichern';b.onclick=downloadDiagnostic;host.appendChild(b)}}if(b){b.disabled=!window.__compositionLabLastDiagnostic;b.title=b.disabled?'Nach der nächsten Komposition verfügbar':'Kompositionsdiagnose als JSON speichern'}return b}
...
function bindMainPlayer(score){try{if(typeof renderMain==='function')renderMain()}catch(_){}try{if(typeof bindPlayerScore==='function')bindPlayerScore('main',score)}catch(_){}const box=document.querySelector('[data-player="main"]');if(!box)return;const bindButtons=()=>{const play=box.querySelector('.play'),pause=box.querySelector('.pause'),stop=box.querySelector('.stop');if(play)play.onclick=()=>{try{const r=window.playPlayer?.('main');if(r?.catch)r.catch(e=>setStatus?.('main',String(e?.message||e),'err'))}catch(e){try{setStatus?.('main',String(e?.message||e),'err')}catch(_){}}};if(pause)pause.onclick=()=>{try{window.pausePlayer?.('main')}catch(e){}};if(stop)stop.onclick=()=>{try{window.stopPlayer?.('main');setStatus?.('main','Gestoppt.')}catch(e){}}};bindButtons();window.addEventListener('compositionlab-samples-ready',()=>{try{bindPlayerScore?.('main',state.current||state.source);bindButtons()}catch(_){}},{once:true})}
function install(){
  const E=window.CompositionLabEngine,btn=$('composeBtn');if(!E||E.BUILD!==14||!btn||typeof state==='undefined')return false;
  simplifyCompositionUI();fixApiKeyFields();const tech=$('techMenu');if(tech&&!tech.dataset.ipadKeyFix){tech.dataset.ipadKeyFix='1';tech.addEventListener('click',()=>setTimeout(fixApiKeyFields,0),true)}
