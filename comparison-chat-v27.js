...
function install(){
 ...
 if(!input||!send||!log)return false;
 if(input.dataset.chatV27)return true;
 input.dataset.chatV27='1';
 ...
 send.onclick=ask;input.disabled=false;input.readOnly=false;input.removeAttribute('inert');input.onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();ask();}};
...
let tries=0;const timer=setInterval(()=>{tries++;const input=$('sourceChatInput');if(input?.dataset?.inputV26&&install())clearInterval(timer);else if(tries>50){install();clearInterval(timer)}},100);
