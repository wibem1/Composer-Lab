(()=>{
'use strict';
const DIAG_VERSION=2, ASSET_VERSION='20260903-22';
...
function save(){if(!store.last)return;snapshot();const text=JSON.stringify(store.last,null,2),name=filename(),type='application/json';try{if(typeof downloadBlob==='function'){downloadBlob(text,name,type);return}}catch(e){}if(window.AndroidBridge?.saveBlob){const r=new FileReader();r.onloadend=()=>window.AndroidBridge.saveBlob(r.result,name,type);r.readAsDataURL(new Blob([text],{type}));return}const a=document.createElement('a'),u=URL.createObjectURL(new Blob([text],{type:'application/json'}));a.href=u;a.download=name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),1500)}
...
const engine=document.createElement('script');engine.src='/Composer-Lab/shared/composition-engine.js?v=20260903-4';engine.onload=()=>{const adapter=document.createElement('script');adapter.src=mode==='root'?'/Composer-Lab/shared/root-engine-adapter.js?v=20260903-28':'/Composer-Lab/shared/rich-engine-adapter.js?v=20260903-4';(document.head||document.body).appendChild(adapter)};(document.head||document.body).appendChild(engine);
})();