(()=>{
'use strict';
const DIAG_VERSION=2, ASSET_VERSION='20260903-14';
...
function clean(x,depth=0){
  ...
  if(typeof x==='object'){
    ...
    for(const [k,v] of Object.entries(x)){
      if(/api.?key|secret|token|authorization|password|\bkeys\b/i.test(k))continue;
      try{o[k]=clean(v,depth+1)}catch(e){}
    }
    return o;
  }
  return String(x);
...
function settings(){
  return {
    provider:getProvider(),model:getModel(),reasoning:val('reasoningStrength','reasoningEffort'),
    measures:val('length','measures'),meter:val('meter'),tempo:val('tempo'),key:val('keySig','musicalKey'),ensemble:val('ensemble'),
    compositionIdea:val('compIdea'),compositionTask:val('compTask'),freePrompt:val('prompt'),sourceName:val('sourceName'),
    templateLength:val('templateLength'),
    ui:{href:location.href,userAgent:navigator.userAgent,platform:navigator.platform||'',language:navigator.language||'',online:navigator.onLine,androidBridge:!!window.AndroidBridge}
  };
}
function currentScore(){
  try{if(typeof lastScore!=='undefined'&&lastScore)return clean(lastScore)}catch(e){}
  try{if(typeof state!=='undefined'&&state.current)return clean(state.current)}catch(e){}
  try{if(typeof mainPlayerScore!=='undefined'&&mainPlayerScore)return clean(mainPlayerScore)}catch(e){}
  return null;
}
function buildLabel(){return $('ipadBuild')?.textContent||document.querySelector('.ipad-build')?.textContent||null}
function begin(){
  const d={format:'composition-lab-diagnostic',diagnosticVersion:DIAG_VERSION,diagnosticsAsset:ASSET_VERSION,uiMode:mode,build:buildLabel(),startedAt:new Date().toISOString(),settings:settings(),calls:[],result:null,error:null};
  store.active=d;store.last=d;updateUi();
...
function callMeta(args){
  if(args.length>=5)return {provider:args[0],model:args[1],systemPrompt:args[3],userPrompt:args[4],wantJson:!!args[5]};
  return {provider:getProvider(),model:getModel(),systemPrompt:args[0],userPrompt:args[1],wantJson:null};
}
try{
  const baseCallLLM=callLLM;
  callLLM=async function(...args){
    const active=store.active;
    const rec=active?{index:active.calls.length+1,startedAt:new Date().toISOString(),...callMeta(args),response:null,error:null}:null;
    if(rec)active.calls.push(rec);
    try{
      const out=await baseCallLLM.apply(this,args);
      if(rec){rec.response=typeof out==='string'?out:clean(out);rec.completedAt=new Date().toISOString();}
      setTimeout(snapshot,0);setTimeout(snapshot,250);setTimeout(snapshot,800);
      return out;
    }catch(e){
      if(rec){rec.error=String(e?.message||e);rec.completedAt=new Date().toISOString();active.error=rec.error;}
      updateUi();throw e;
    }
  };
}catch(e){console.warn('Composition Lab diagnostics: callLLM konnte nicht protokolliert werden',e)}
